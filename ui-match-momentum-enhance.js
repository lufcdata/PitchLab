(()=>{
  ...
  const hasQualifier=(e,...names)=>{const q=qualifierNames(e);return names.some(n=>q.includes(String(n).toLowerCase().replace(/[\s_-]/g,'')));};
  ...
  const evtSec=e=>{
    ...
    const t=canonical?.timelineSecond?.(e);
  ...
  const minuteLabel=e=>{
    const m=Number(e?.expandedMinute??e?.minute??0);
  ...
  const weight=e=>{
    ...
    if(t==='pass'&&ok){let w=endX>=83?1.35:(endX>=67?.7:0);if(hasQualifier(e,'KeyPass','Assist','IntentionalGoalAssist'))w+=2.2;return w;}
  ...
  function scoreAfter(target,ordered,home,away){
    ...
    for(const e of ordered){
      if(isGoal(e)){
        const tm=goalTeam(e,home,away);if(tm===home)h++;else if(tm===away)a++;
      }
      if(e===target)break;
    }
    return `${h}–${a}`;
  ...
  function tooltipHtml(e,ordered,home,away){
    const goal=isGoal(e),red=isRed(e),credited=goal?goalTeam(e,home,away):teamOf(e),name=playerName(e),minute=minuteLabel(e);
    const penalty=goal&&hasQualifier(e,'Penalty');
    const own=goal&&isOwnGoal(e);
    const secondYellow=red&&hasQualifier(e,'SecondYellow','SecondYellowRed');
    let extra='';
    if(goal){
      if(penalty)extra+='<span>Penalty</span>';
      if(own)extra+='<span>Own goal</span>';
      extra+=`<strong>Score: ${scoreAfter(e,ordered,home,away)}</strong>`;
    }else if(secondYellow){extra+='<span>Second yellow card</span>';}
    return `<b>${goal?'⚽ Goal':'🟥 Red card'} — ${name}</b><span>${minute}${minute?' · ':''}${credited||'Unknown team'}</span>${extra}`;
  }
  function ensureTooltip(chart){
    let tip=chart.querySelector('.match-momentum__tooltip');
    if(!tip){tip=document.createElement('div');tip.className='match-momentum__tooltip';tip.setAttribute('role','status');chart.appendChild(tip);}return tip;
  }
  function bindMarkers(svg,ordered,home,away){
    const chart=svg.closest('.match-momentum__chart');if(!chart)return;
    const tip=ensureTooltip(chart);
    const hide=()=>tip.classList.remove('is-visible');
    const show=(el,e)=>{
      tip.innerHTML=tooltipHtml(e,ordered,home,away);
      const chartRect=chart.getBoundingClientRect(),markRect=el.getBoundingClientRect();
      const x=markRect.left-chartRect.left+markRect.width/2,y=markRect.top-chartRect.top;
      tip.style.left=`${Math.max(8,Math.min(chartRect.width-230,x-100))}px`;
      tip.style.top=`${Math.max(8,y-74)}px`;
      tip.classList.add('is-visible');
    };
    svg.querySelectorAll('.match-momentum__marker').forEach((el,i)=>{const e=ordered.filter(q=>isGoal(q)||isRed(q))[i];if(!e)return;el.addEventListener('mouseenter',()=>show(el,e));el.addEventListener('mouseleave',hide);el.addEventListener('focus',()=>show(el,e));el.addEventListener('blur',hide);});
  }
  function render(){
    ...
    const homePath=`M ${left} ${mid} ${curve(homePts).replace(/^M [^C]+?/,'').trim()} L ${left+plotW} ${mid} Z`;
    const awayPath=`M ${left} ${mid} ${curve(awayPts).replace(/^M [^C]+?/,'').trim()} L ${left+plotW} ${mid} Z`;
    ...
    bindMarkers(svg,ordered,home,away);
    svg.dataset.momentumEnhanced='1';
  }
  let timer=0;
