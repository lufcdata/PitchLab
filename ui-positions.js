(()=>{
  ...
  positionsToggle.addEventListener('click',()=>setActive(!active));statsToggle?.addEventListener('click',()=>{if(active)setActive(false)},true);[from,to,teamEl].forEach(el=>{el.addEventListener('input',render);el.addEventListener('change',render)});document.addEventListener('pitchlab:canonical-time-ready',render);document.addEventListener('pitchlab:match-loaded',()=>requestAnimationFrame(render));new MutationObserver(render).observe(eventCount,{childList:true,characterData:true,subtree:true});
})();