(()=>{
  ...
  function addBoundary(points,colour,kind){if(points.length<3)return;const d=pathFor(dominantCluster(points));if(!d)return;const path=document.createElementNS(svg.namespaceURI,'path');path.setAttribute('d',d);path.setAttribute('class',`pitch-actions-map__hull is-${kind}`);path.style.setProperty('--action-colour',colour);svg.appendChild(path)}
  ...
  window.PitchLabActionsMap=Object.freeze({version:'ACTIONS_MAP_V1_2_2026-09-11',render:schedule,setMode});
})();