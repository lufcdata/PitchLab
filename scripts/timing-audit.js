...
if(bt?.ballStatusMethod!=='event-restart-estimate')throw new Error('Ball-status method must be explicitly estimated');
...
if(!ft||ft.version!=='MATCH_TIMINGS_DYNAMIC_V1_2026-09-06'||ft.matchDuration<5400||ft.gameStops<1)throw new Error('Forest dynamic timing control failed');
if(JSON.stringify(ft)===JSON.stringify(bt))throw new Error('Fixture switch did not change timing population');
console.log('PASS: Match Timings are fixture-derived; Bournemouth exact controls and Forest fixture differentiation passed.');
