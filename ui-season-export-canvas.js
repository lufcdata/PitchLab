(()=>{
  ...
  function legendItems(){const root=$('plotLegend');if(!root)return[];const items=[];let metric='';for(const node of root.children){const text=(node.textContent||'').trim();if(!text)continue;if(node.classList?.contains('season-metric-legend-label')){metric=text;continue}let label=text,noun=legendNoun(metric);if(/^Successful$/i.test(text)&&noun)label=`Successful ${noun}`;else if(/^Unsuccessful$/i.test(text)&&noun)label=`Unsuccessful ${noun}`;const marker=node.querySelector?.('.legend-arrow,.legend-circle'),colour=marker?.style?.getPropertyValue('--metric-colour')?.trim()||'';if(label&&!items.some(x=>x.text===label))items.push({text:label,colour:colour||null})}return items.slice(0,12)}
  function drawLegends(ctx){const items=legendItems();if(!items.length)return;ctx.font=font(700,11);const markerW=34,gap=14,itemPad=24,maxRowW=690,rows=[];let row=[],rowW=0;for(const item of items){const itemW=markerW+gap+ctx.measureText(item.text).width+itemPad;if(row.length&&rowW+itemW>maxRowW){rows.push({items:row,width:rowW});row=[];rowW=0}row.push({...item,width:itemW});rowW+=itemW}if(row.length)rows.push({items:row,width:rowW});const rowH=18,totalH=rows.length*rowH,startY=1208+Math.max(0,(42-totalH)/2);rows.forEach((r,ri)=>{let x=540-r.width/2;const y=startY+ri*rowH;r.items.forEach((item,ii)=>{const colour=item.colour||(ii%2?'#aeb5c4':'#4ef0ce');ctx.strokeStyle=colour;ctx.fillStyle=colour;ctx.lineWidth=2.5;ctx.beginPath();ctx.moveTo(x,y);ctx.lineTo(x+26,y);ctx.stroke();ctx.beginPath();ctx.arc(x+31,y,3.5,0,Math.PI*2);ctx.fill();ctx.fillStyle='#aeb5c4';ctx.fillText(item.text,x+markerW+gap,y+4);x+=item.width})})}
  ...
  window.PitchLabSeasonExportCanvas=Object.freeze({version:'SEASON_EXPORT_SHOT_LEGENDS_V4_2026-09-09',template:TEMPLATE,width:WIDTH,height:HEIGHT,exportScale:EXPORT_SCALE,exportWidth:WIDTH*EXPORT_SCALE,exportHeight:HEIGHT*EXPORT_SCALE,brandingTop:BRANDING_TOP,pitch:PITCH,exportPng});
})();